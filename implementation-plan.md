...
**Phase**: Backend Data Model - In Progress (PR13 Complete, PR14 Ready to Start)
**Last Completed PR**: PR13 - Add Pydantic Models & Basic Entity Generation (Completed 2025-11-12)
**Next Up**: PR14 - Add ProductionBatch Model & Generation
...
## Active PR (Ready to Start)

### PR14: Add ProductionBatch Model & Generation
...
#### Tasks
- [ ] Add ProductionBatch Pydantic model to shared/models.py
  - **Priority**: Critical
  - **Effort**: Medium (1-2hrs)
  - **Context**: Detailed production batch with traceability links
  - **Fields**: batch_id, order_id, part_number, shift, operator, parts_produced, good_parts, scrap_parts, serial_range, materials_consumed, process_parameters

- [ ] Add MaterialUsage nested model for materials_consumed[]
  - **Priority**: Critical
  - **Effort**: Quick (<1hr)
  - **Context**: Track which material lots used in batch
  - **Fields**: material_id, material_name, lot_number, quantity_used, unit

- [ ] Implement generate_production_batches() in data_generator.py
  - **Priority**: Critical
  - **Effort**: Large (3-4hrs)
  - **Context**: Convert daily production totals into ~18 batches/day (2 shifts × 6 machines × 1.5 batches/shift)
  - **Logic**: Split by shift, assign serial ranges, link to orders and materials
  - **Output**: ~540 batches for 30 days

- [ ] Add batch assignment logic
  - **Priority**: Critical
  - **Effort**: Medium (2-3hrs)
  - **Context**: Assign batches to orders (round-robin or priority-based)
  - **Logic**: Match part numbers, respect order quantities and due dates

- [ ] Add material selection logic
  - **Priority**: Critical
  - **Effort**: Medium (2-3hrs)
  - **Context**: Select material lots for batch based on machine type and availability
  - **Logic**: Match material requirements, deplete lot quantities, respect quarantine status

- [ ] Move quality_issues from production[date][machine] to batches
  - **Priority**: Critical
  - **Effort**: Medium (1-2hrs)
  - **Context**: Quality issues belong to specific batches, not daily aggregates
  - **Logic**: Assign each quality issue to a batch, link to materials in that batch

- [ ] Update shared/data.py to add batches[] to production[date][machine]
  - **Priority**: Critical
  - **Effort**: Medium (1-2hrs)
  - **Context**: Add batches as new field in production structure
  - **Note**: Keep existing fields (oee, availability, etc.) for backward compatibility

- [ ] Create test dataset (2 days, 2 machines) for validation
  - **Priority**: High
  - **Effort**: Medium (1-2hrs)
  - **Context**: Small dataset to manually verify relationships
  - **Tests**: Verify batch → order links, batch → material lot links, quality issues in batches

- [ ] Add tests for batch generation
  - **Priority**: High
  - **Effort**: Medium (1-2hrs)
  - **Context**: Test batch assignment, material selection, serial range generation

#### Acceptance Criteria
- ProductionBatch model validates correctly
- generate_production_batches() creates ~18 batches per day
- Each batch links to valid order_id
...
- Small test dataset (2 days, 2 machines) validates all relationships
- All tests pass
- Data file size ~120KB for batches (manageable)

---

## Planned PRs (In Priority Order)

### PR15: Aggregation Function & Backward Compatibility
**Priority**: Critical
**Estimated Effort**: 6-8 hours
...
#### Tasks
- [ ] Create aggregate_batches_to_production() function in shared/data.py
  - **Priority**: Critical
  - **Effort**: Large (3-4hrs)
  - **Context**: Derive production[date][machine] metrics from production_batches[]
  - **Logic**: Sum parts_produced, calculate OEE, aggregate quality issues, derive availability/performance

- [ ] Implement OEE calculation from batches
  - **Priority**: Critical
  - **Effort**: Medium (1-2hrs)
  - **Context**: Calculate OEE, availability, performance from batch-level data
  - **Formula**: OEE = availability × performance × quality

- [ ] Implement quality metrics aggregation from batches
  - **Priority**: Critical
  - **Effort**: Medium (1-2hrs)
  - **Context**: Aggregate defect_count, scrap_rate from batch quality issues
  - **Logic**: Sum across all batches for day/machine

- [ ] Implement shift aggregation from batches
  - **Priority**: High
  - **Effort**: Medium (1-2hrs)
  - **Context**: Aggregate shift-level metrics (Day/Night) from batches
  - **Logic**: Group batches by shift, sum metrics

- [ ] Update generate_production_data() to use batch generation + aggregation
  - **Priority**: Critical
  - **Effort**: Medium (1-2hrs)
  - **Context**: Change data generation flow: generate batches → aggregate to production
  - **Note**: production[date][machine] becomes derived view, not source of truth

- [ ] Test aggregated data matches expected format
  - **Priority**: Critical
  - **Effort**: Medium (1-2hrs)
  - **Context**: Verify aggregated production has same structure as old generation
  - **Tests**: Field existence, value ranges, backward compatibility

- [ ] Test existing shared/metrics.py functions still work
  - **Priority**: Critical
  - **Effort**: Medium (1-2hrs)
  - **Context**: Ensure calculate_oee_metrics(), get_quality_summary() work unchanged
  - **Tests**: Run all existing metric calculations on new data structure

- [ ] Add integration tests for batch → production aggregation
  - **Priority**: High
  - **Effort**: Medium (1-2hrs)
  - **Context**: End-to-end test: generate batches → aggregate → validate production structure

#### Acceptance Criteria
- aggregate_batches_to_production() produces production[date][machine] structure
- Aggregated metrics match batch-level data (totals, OEE calculations)
- Existing metrics.py functions work unchanged
- Existing backend API responses unchanged
- Existing frontend displays work unchanged
- All tests pass
- No regressions in existing functionality
- Data generation flow: batches first, production derived
